...
class Tasks_bluebird extends Tasks
{
	...
	/**
	 * Fetch a user's timeline from either the cache or from the API
	 **/
	public function getTweets($params)
	{
		$this->params = $params;
		$cache_file = 'timeline/'.$this->params['screen_name'];

		// Check the cache before continuing. We don't want to hit the API on every request.
		$cached_tweets = $this->cache->getYAML($cache_file);
		// If there's a cache and it's older than our specified time, delete it. It'll be recreated later.
		if ($cached_tweets) {
			if ($this->cache->getAge($cache_file) >= $this->params['cache_length']) {
				$this->cache->delete($cache_file);
			}
		...
		// Get tweets from API
		$tweets = $this->getTweetsFromAPI();
		// Place tweets for user into their own cache file
		$this->cache->putYAML($cache_file, $tweets);
		// Send them back
		return $tweets;
	}
	...
	public function getTweet($id)
	{
		...
		// Check the cache before continuing. We don't want to hit the API on every request.
		$cached_tweet = $this->cache->getYAML($cache_file);
		// If there's a cache, use that.
		if ($cached_tweet) {
			return $cached_tweet;
		}
		...
		$tweet = json_decode($this->performRequest($url, array('id' => $id)), true);
		// Place tweet into its own cache file
		$this->cache->putYAML($cache_file, $tweet);
		// Send it back
		return $tweet;
	}
